...
require.extensions[".ts"] = (module, filename) => { let source = fs.readFileSync(filename, "utf8").replaceAll("import.meta.env.DEV", "false"); const output = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2020, esModuleInterop: true, resolveJsonModule: true } }); module._compile(output.outputText, filename); };
require("./test-assessment-flow.ts");
