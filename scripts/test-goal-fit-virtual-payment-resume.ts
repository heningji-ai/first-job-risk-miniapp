...
void (async () => {
  ...
  clears=0;const unavailable=await resume.resumeGoalFitVirtualPaymentConfirmation({assessmentId:"asm_1",dependencies:{...base,fetchFullReport:async()=>{throw new Error("temporary")}}});assert(unavailable?.status==="entitled_pending"&&unavailable.safeCode==="FULL_REPORT_TEMPORARILY_UNAVAILABLE"&&clears===0,"paid report unavailable retains record for direct report recovery");
