...
try {
  ...
  await ensureWechatMiniappSessionWith({ platform: "wechat_miniapp", read: () => { throw new Error("storage"); }, save: () => undefined, login: () => { throw new Error("must not login"); }, exchange: async () => { throw new Error("unexpected"); }, visitorId: () => "visitor_12345678" });
  await ensureWechatMiniappSessionWith({ platform: "wechat_miniapp", read: () => null, save: () => { throw new Error("storage"); }, login: ({ success }: any) => success({ code: "new" }), exchange: async () => ({ sessionToken: "fresh", expiresAt: future(120_000), visitorId: "visitor_12345678" }), visitorId: () => "visitor_12345678" });
  await ensureWechatMiniappSessionWith({ platform: "wechat_miniapp", read: () => null, save: () => undefined, login: ({ fail }: any) => fail(), exchange: async () => { throw new Error("unexpected"); }, visitorId: () => "visitor_12345678" });
  await ensureWechatMiniappSessionWith({ platform: "wechat_miniapp", read: () => null, save: () => undefined, login: ({ success }: any) => success({ code: "new" }), exchange: async () => { throw new Error("api"); }, visitorId: () => "visitor_12345678" });
