...
for (const token of ["fetchGoalFitPurchases", "GoalFitPurchasesContractError", "forceRefreshWechatMiniappSession", "retryCount === 0", "auth_error", "network_error", "server_error", "contract_error", "parserStage", "responseRootKeys", "purchasesType", "invalidItemIndex", "invalidFieldNames", "invalidFieldTypeMap", "totalItemCount", "validItemCount", "invalidItemCount", "invalidAssessmentIdSuffix", "invalidReportSnapshotIdSuffix", "purchases_partial_contract_error", "暂无报告记录", "登录状态恢复失败", "网络连接异常", "报告数据暂时无法读取", "reportTypeTitle", "companyType", "roleName", "completedAt", "primaryConclusion", "item.unlocked", "item.status === 'REFUNDED'", "已退款", "重新加载", "assessmentId=${encodeURIComponent(item.assessmentId)}", "source=my-reports"]) assert(list.includes(token), `my reports behavior missing ${token}`);
