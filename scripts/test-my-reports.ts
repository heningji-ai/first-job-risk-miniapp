...
for (const token of ["fetchGoalFitPurchases", "onShow", "reportTypeTitle", "companyType", "roleName", "completedAt", "primaryConclusion", "item.unlocked", "item.status === 'REFUNDED'", "已退款", "重新加载", "assessmentId=${encodeURIComponent(item.assessmentId)}", "source=my-reports"]) assert(list.includes(token), `my reports behavior missing ${token}`);
assert(routes.includes("pages/my-reports/index"), "my reports route missing");
for (const token of ["historyMode", "loadHistory", "readGoalFitHistoryReportRecovery", "saveGoalFitHistoryReportRecovery", "canPurchaseThisHistoryReport", "REFUNDED", "requested||recovery", "assessmentId.value!==requestedId", "ENTITLED_TEMPORARY_UNAVAILABLE"]) assert(detail.includes(token), `history detail behavior missing ${token}`);
