...
assert(source.includes("关注猎头季哥服务号，继续获得求职帮助") && source.includes("service-account-entry-free") && source.includes("service-account-entry-arrow"), "the free-result service-account entry must be an explicit lightweight CTA");
assert(source.includes("border:1rpx solid #d7dff4") && source.includes("border-radius:999rpx") && source.includes("background:#f5f7ff"), "the free-result service-account CTA must retain the lightweight outlined pill affordance");
