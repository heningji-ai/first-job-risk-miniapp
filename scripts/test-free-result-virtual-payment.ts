...
for (const token of ["解锁完整报告 ¥19.9", "getPlatform()==='wechat_miniapp'", "startManagedGoalFitVirtualPayment", "subscribeGoalFitVirtualPaymentState", "invalidateGoalFitVirtualPaymentFlow", "paymentState.value.busy", "fullReport.value=outcome.report", "onUnload"]) assert(source.includes(token), `missing ${token}`);
