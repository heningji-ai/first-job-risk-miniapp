...
assert(source.includes("关注猎头季哥服务号，继续获得求职帮助") && source.includes("sticky-service-account-button") && source.includes("service-account-entry-arrow"), "the free-result service-account entry must be an explicit lightweight CTA inside the sticky purchase bar");
assert(source.indexOf("sticky-service-account-button") > source.indexOf('class="fixed-cta"') && !source.includes("inline-service-account-button") && !source.includes("一次购买，长期查看本次报告"), "the free-result service-account CTA must replace the weak sticky copy without rendering a duplicate body-card entry");
assert(source.includes(".content{padding-bottom:292rpx}") && source.includes("border:1rpx solid #d7dff4") && source.includes("border-radius:14rpx") && source.includes("background:#f5f7ff"), "the sticky secondary CTA must reserve content space and retain the lightweight outlined-button affordance");
