...
assert(source.includes("关注猎头季哥服务号，继续获得求职帮助") && source.includes("inline-service-account-button") && source.includes("service-account-entry-arrow"), "the free-result service-account entry must be an explicit lightweight CTA inside the purchase area");
assert(source.indexOf("inline-service-account-button") > source.indexOf('class="inline-purchase card"') && !source.includes("inline-purchase-copy") && !source.includes("service-account-entry-free"), "the free-result service-account CTA must replace the weak purchase copy rather than render as a duplicate card-external entry");
assert(source.includes("border:1rpx solid #d7dff4") && source.includes("border-radius:16rpx") && source.includes("background:#f5f7ff"), "the inline service-account CTA must retain the lightweight outlined secondary-button affordance");
